...
export const ButtonHeadTable = ({ sortSelected, toggleSortSelected, name, label, start, end, textCenter, empty }: props) => {
    const baseStyleTh = `py-1.5 text-[#004C4C] bg-[#CDECEC] ${textCenter ? "text-center" : "text-left "}`;
    const startStyleTh = baseStyleTh + " rounded-l-lg pl-4";
    const endStyleTh = baseStyleTh + " rounded-r-lg pr-4";

    if (empty) return <th className={`${start ? startStyleTh : end ? endStyleTh : baseStyleTh}`}/>;

    return (
        <th className={`${start ? startStyleTh : end ? endStyleTh : baseStyleTh}`}>
            <button
