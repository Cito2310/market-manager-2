...
export const createProductMiddlewares = [
    ...
    check("info.size").trim().isLength({max: 24}).withMessage("size length can only be less than 24 characters"),
    ...
    check("info.unitType").isString().trim().notEmpty().withMessage("unitType is required"),
    check("info.unitType").trim().isIn(["unit", "weight"]).withMessage("unitType invalid [ weight | unit ]"),
    ...
    check("info.location").custom(basicValidationsString("info.location")),
